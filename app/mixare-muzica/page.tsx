  ...
  return (
    <ul className="space-y-1">
      {nodes.map((node) => (
        <li key={node.slug}>
          <Link
            href={`/${SERIES}/${getFirstLeafSlug(node)}`}
...
export default async function TutorialeIndexPage() {
  const tutorials = await getAllTutorials(SERIES);
  ...
  return (
    <main className="rounded-xl border border-slate-200 bg-white p-6 text-slate-900 shadow-sm">
      <h1 className="text-3xl font-bold text-slate-950">Mixare Muzica</h1>
      <p className="mt-2 text-base leading-relaxed text-slate-700 md:text-lg">
        Tehnici si explicatii practice pentru mixare, prezentate simplu si
        aplicat.
      </p>
      ...
      <div className="mt-6 space-y-3">
        {tutorialTree.map((item) => (
          ...
          >
            ...
            <Link
              href={`/${SERIES}/${getFirstLeafSlug(item)}`}
