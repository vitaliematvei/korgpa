  ...
  return (
    <ul className="space-y-1">
      {nodes.map((node) => (
        <li key={node.slug}>
          <Link
            href={`/programare/${getFirstLeafSlug(node)}`}
...
export default async function ProgramareIndexPage() {
  ...
  return (
    <main className="rounded-xl border border-slate-200 bg-white p-6 text-slate-900 shadow-sm">
      <h1 className="text-3xl font-bold text-slate-950">Programare</h1>
      ...
      <div className="mt-6 space-y-3">
        {tutorialTree.map((item) => (
          ...
          >
            ...
            <Link
              href={`/programare/${getFirstLeafSlug(item)}`}
